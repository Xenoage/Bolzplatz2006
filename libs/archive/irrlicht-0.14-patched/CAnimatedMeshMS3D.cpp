...
bool CAnimatedMeshMS3D::loadFile(io::IReadFile* file)
{
  ...
  //IRRLICHT 0.12 CODE - OBSOLETE <<< */
	
	
	//ANDI'S .MS3D LOADER PATCH >>>
	...
	for (s32 i = 0 ; i < numTriangles; i++)
	{
      ...
      //collect vertex data
			...
			s32 index = -1;
			for (s32 iV = 0; iV < Vertices.size(); iV++)
			{
        if (v == Vertices[iV])
        {
          index = iV;
      ...
      if (index == -1)
      {
        //this is a vertex we never used before, add it.
        s32 boneid = vertices[triangles[i].VertexIndices[j]].BoneID;
			  if (boneid>=0 && boneid<(s32)Joints.size())
				  Joints[boneid].VertexIds.push_back(Vertices.size());
        Vertices.push_back(v);
  ...
  s32 iIndex = 0;
	...
	{ 
		...
		//SDI2000'S .MS3D 0-MATERIAL PATCH (part 2) >>>
	  //( http://irrlicht.sourceforge.net/phpBB2/viewtopic.php?t=8411 )
	  if (grp.MaterialIdx >= Buffers.size())
	  {
      grp.MaterialIdx = 0;
    }
