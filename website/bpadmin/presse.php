...
<span class="head"><B>BP2k6</B> -&gt; Admin -&gt; Presse</B></span>
</TD></TR>
...
$AddNow = "INSERT INTO presse (Datum, Head_D, Text_D, Head_E, Text_E, Head_F, Text_F, Autor, Bild )
                       VALUES (
                              '$timenow',
                              '$_POST[head_d]',
                              '$_POST[text_d]',
...
echo '<META HTTP-EQUIV="Refresh" CONTENT="1; URL=presse.php?'.$ProID.'">';
    }
...
if ($_POST["do"] == "delete")        # Eintrag loeschen?
    ...
    $GetSql = mysql_query("SELECT Bild FROM presse WHERE NewsID='$ClrID'");
    $XTemp = mysql_fetch_row($GetSql);
    $ClrPict = $XTemp[0];
    if ($ClrPict != "")
        {
        $OldFile = "../global/grafik/presse/".$ClrPict; unlink ($OldFile);
        $OldFile = "../global/grafik/presse/pv_".$ClrPict; unlink ($OldFile);
        }

    mysql_query("DELETE FROM presse WHERE NewsID='$ClrID' LIMIT 1");
...
if ($file_name != "")
    ...
    if ($_POST["OrgPict"] != "")
        {
        $OldFile = "../global/grafik/presse/".$_POST["OrgPict"]; unlink ($OldFile);
        $OldFile = "../global/grafik/presse/pv_".$_POST["OrgPict"]; unlink ($OldFile);
        }
...
## ----------------------------------------------------------------------------- presse ausgeben (wenn _nicht_ im EDIT Fenster)

if ($_POST["do"] != "edit") {
echo '<B>Presse:</B><P>';
$GetSql = mysql_query("SELECT * FROM presse ORDER BY NewsID desc");
...
while ($data = mysql_fetch_row($GetSql))
       ...
       echo '<FORM ACTION METHOD="POST" ACTION="presse.php"><TD VALIGN="Top" ALIGN="Center" class="'.$MyCol.'"><input type="hidden" name="VarID" value="'.$data[0].'"><input type="hidden" name="do" value="edit"><input type="submit" value="edit" class="EditBtn"></input></TD></FORM>'."\n";
       echo '<TD width="1" class="'.$MyCol.'">&nbsp;</TD>'."\n";
       echo '<FORM ACTION METHOD="POST" ACTION="presse.php"><TD VALIGN="Top" ALIGN="Center" class="'.$MyCol.'"><input type="hidden" name="VarID" value="'.$data[0].'"><input type="hidden" name="do" value="delete"><input type="submit" value="DEL" class="DelBtn" onClick="return confirm(\'[ACHTUNG] Soll der Eintrag\n\n '.$MyHeadline.'\n\nunwiderruflich gelöscht werden ?\')"></input></TD></FORM></TR>'."\n";
       }
...
<B>Neuen Artikel anlegen:<P>Autor:
...
<TD><button name="sende" type="submit">Artikel eintragen!</button></TD></TR>
</TABLE>
<HR SIZE="1">
</FORM>
...
#______________________________________________________________________  ####### presse Aendern

function EditEntry($AktPictFile) {
$EditID = $_POST["SaveID"];
$SaveEdit = "UPDATE presse SET Head_D='$_POST[head_d]', Text_D='$_POST[text_d]', Head_E='$_POST[head_e]', Text_E='$_POST[text_e]', Head_F='$_POST[head_f]', Text_F='$_POST[text_f]', Bild='$AktPictFile'  WHERE NewsID='$EditID' LIMIT 1";
...
$SaveDir = "../global/grafik/presse";    # in welches Verzeichnis speichern
...
if ($_POST["do"] == "edit")
    {
    $GetSql = mysql_query("SELECT * FROM presse WHERE NewsID='$EditID'");
    $XOrg = mysql_fetch_row($GetSql);
               ...
               if ($XOrg[9] != "")
                   {
                   echo '<TD VALIGN="Top"><img src="../global/grafik/presse/pv_'.$XOrg[9].'" border="1"></TD>';
                   echo '<TD>&nbsp;&nbsp;&nbsp;&nbsp;</TD>';
                   }
               echo '<TD VALIGN="Top"';
...
<TD VALIGN="Middle" class="tabletd"><input type="text" name="head_d" size="80" maxlength="200" value="<? echo $XOrg[2]; ?>"></TD></TR>
<TR>
...
<TR>
<TD VALIGN="Middle" class="tabletd">Überschrift:</TD>
<TD VALIGN="Middle" class="tabletd"><input type="text" name="head_e" size="80" maxlength="200" value="<? echo $XOrg[4]; ?>"></TD></TR>
<TR>
<TD VALIGN="Top" class="tabletd">Text:</TD>
...
<TR>
<TD VALIGN="Middle" class="tabletd">Überschrift:</TD>
<TD VALIGN="Middle" class="tabletd"><input type="text" name="head_f" size="80" maxlength="200" value="<? echo $XOrg[6]; ?>"></TD></TR>
<TR>
<TD VALIGN="Top" class="tabletd">Text:</TD>
