...
$MaxChars = $XTemp[8];                     ## hole MaxChars fuer Zeichenbegrenzung
...
$GetSql = mysql_query("SELECT * FROM presse ORDER BY NewsID desc");               # hole alle News..
while ($data = mysql_fetch_row($GetSql))                                        # ..Ausgabe aller News
       ...
       if ($data[$LanguageID] != "")
           ...
           */
           ...
           if ($data[9] != "")
               ...
               ?><a href="javascript:showPicture2('../global/grafik/presse/<? echo $data[9]; ?>','-');"><?
               echo '<img src="../global/grafik/presse/pv_';
       ...
       {
       $GetSql = mysql_query("SELECT * FROM presse WHERE NewsID='$_GET[id]'");    # hole gewaehlte presse
